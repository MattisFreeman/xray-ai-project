...
configure () {
    ...
    local hooks=(  'entering_cmd'
     ...
     case "$1" in
       ...
       xai_timeout)            eval "$1=\"$(dialog_input "Delay during voice command input before timeout\nDefault: 10" "${!1}")\"";;
       xai_use_bluetooth)      eval "$1=\"$(dialog_yesno "Do you want to use a bluetooth Speaker?\nThis will start PulseAudio" "${!1}")\""
                              $xai_use_bluetooth && pulseaudio --start || pulseaudio --kill
                              ;;
       ...
       load)
           ...
           if [ "$tts_engine" == "voxygen" ]; then
               xai_error "Voxygen speech engine has been removed as no longer supported"
               xai_debug ""
               xai_warning "Change your speech engine in Settings > Speech synthesis"
           fi
       ...
       play_hw)
           ...
           while true; do
               ...
               if dialog_yesno "Checking audio output, make sure your speakers are on and press [Yes].\nPress [No] if you don't have speakers." true >/dev/null; then
                   ...
                   clear
                   xai_warning "Selection of the speaker device"
                   aplay -l
       ...
       rec_hw)
           ...
           while true; do
               if dialog_yesno "Checking audio input, make sure your microphone is on, press [Yes] and say something.\nPress [No] if you don't have a microphone." true >/dev/null; then
                   ...
                   xai_warning "Selection of the microphone device"
                   arecord -l
                   read -p "Indicate the card # to use [0-9]: " card
                   read -p "Indicate the device # to use [0-9]: " device
       ...
       tts_engine)          options=('svox_pico' 'google' 'espeak' 'osx_say')
                            ...
                            rm -f "$xai_cache_folder"/*.mp3 # remove cached voice
                            case "$tts_engine" in
       ...
       *)                   xai_error "ERROR: Unknown configure $1";;
   esac
   return 0
}
...
EOM
   ...
   # if has mic
   if [ -n "$rec_hw" ]; then
       xai_auto_levels # adjust audio levels only if mic is present
